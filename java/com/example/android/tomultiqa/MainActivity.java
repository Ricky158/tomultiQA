...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        MainLoad1.start();
        //2.load tasks which need to change UI.
        ...
        MainLoad4.start();
        //3.load data without UI change and no between relationship in another thread.
        Thread MainLoad5 = new Thread(() -> {//load data without UI change and no between relationship in another thread.
    ...
    private final Handler handler = new Handler(msg -> {
        switch (msg.what){
            ...
            case USER_IMPORT_DONE:
                ...
                CheckLevel();//EXP data will be loaded in another Thread, but UI changes loading in here.
    ...
    private void ImportConflictBossData(boolean IsRebornMode){
        ...
        if(BossRebornTime > 0 && BossState > 0){
            ...
            handler.sendMessage(message);// TODO: 2021/8/1
        }
    }
    ...
    private void SetBossValues(String name, int Level, int HP, int SD, int Turn, int EXPReward, int MaterialReward, int PointReward){
        ...
        LevelDamageChange = 1.0 + (UserLevel - BossLevel) * 0.03;
