...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        MainLoad1.start();
        //2.load data without UI change and no between relationship in another thread.
        Thread MainLoad2 = new Thread(() -> {//load data without UI change and no between relationship in another thread.
        ...
        MainLoad3.start();
        Thread MainLoad4 = new Thread(() -> {
            //3.load tasks which need to change UI.
    ...
    private final Handler handler = new Handler(msg -> {
        switch (msg.what){
            ...
            case BOSS_IMPORT_DONE:
                //continue load user data(including battle UI changes.)
            ...
            case USER_IMPORT_DONE:
                //continue load EXP and Answering data(including UI changes.)
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        if (item.getItemId() == R.id.action_copy) {//if setting icon in Menu be touched.
            ...
            startActivity(i);
        }else if(item.getItemId() == R.id.action_favorite && FavoriteNumber > 0){
    ...
    public void ReloadQuest(View view){
        ...
        Button FavoriteQuestButton = findViewById(R.id.FavoriteQuestButton);
    ...
    private void BattleCalculation(boolean IsQuestRight, boolean IsBossHere){
        ...
        if (IsBossHere && BattleTurn > 0 && BossNowHP > 0){
            ...
            if(IsCritical){
                CurrentDamage = UserATK * UserCriticalDamage / 100 * (1 + LevelDamageChange);
            }else{
                CurrentDamage = UserATK * (1 + LevelDamageChange);
            }
            ...
            handler.postDelayed(() -> DamageShowView.setText("--"), 500);
    ...
    private void CloseBattle(boolean IsWin){
        ...
        if (IsWin && AppMode.equals(ValueClass.GAME_MODE)) {
            ...
            SupportClass.saveIntData(this,"RecordDataFile","BattleFail",
                    SupportClass.getIntData(this,"RecordDataFile","BattleFail",0) + 1);
    ...
    private void SetBossValues(String name, int Level, int HP, int SD, int Turn, int EXPReward, int MaterialReward, int PointReward){
        ...
        //ConstraintLayout BossShieldLayout = findViewById(R.id.BossShieldLayout);todo
...
//                }//not necessary, we can edit Turn number in boss design.
                ...
                case "Calculate":
                    ...
                    if(BossAbilityText.contains(getString(R.string.FearAbilityTran)) && (float) CurrentDamage / BossTotalHP > 0.25){
                        CurrentDamage = BossTotalHP * 0.25;
                    }
                    break;
    ...
    public void ChangeTakeNoteState(View view){
        ...
        if(IsTakeNoteEnabled){
            ...
            Toast.makeText(getApplicationContext(), getString(R.string.NoteInputStartHintTran), Toast.LENGTH_SHORT).show();
        }else{
    ...
    private void ChangeQuestTotalNumber(){
        ...
        //3.load database and put in variable.
        //put in MainLoad2 Thread to load.
        //No need to database to change UI.
