...
public class MarketActivity extends AppCompatActivity {
    ...
    private void MarketPriceChange(){
        ...
        if(SupportClass.CreateRandomNumber(1,100) < 3){//2% Unusual possibility.
            IsUsualDay = false;
        }
        ...
        if(IsUserWealthy && IsUsualDay){
            PriceIndex = (double) SupportClass.CreateRandomNumber(-50,50) / 100;
        }else if(IsUserWealthy){//IsUsualDay is false.
            PriceIndex = (double) SupportClass.CreateRandomNumber(-200,200) / 100;
        }else if(IsUsualDay){//IsUserWealthy is false.
            PriceIndex = (double) SupportClass.CreateRandomNumber(-120,120) / 100;
        }else{//IsUsualDay and IsUserWealthy are all false.
            PriceIndex = (double) SupportClass.CreateRandomNumber(-25,25) / 100;
        }
        //4.calculate Price changed, but not change the actual value yet.
        BoxPriceChanged = (int) (BoxMarketPrice * (1 + PriceIndex) );
        KeyPriceChanged = (int) (KeyMarketPrice * (1 + PriceIndex) );
        ...
        KeyMarketPriceView.setText(SupportClass.ReturnKiloIntString(BoxPriceChanged) + " " + getString(R.string.PointPerKeyTran));
    ...
    private void InitializingDataBase(){
        ...
        //4.1 get data from database to Cursor.
        SearchDataBase(null,null);
        //4.2 if no item record in database, create a new record(all items).
        if(ResultCursor.getCount() <= 0){
            NewItemColumn(EpicItemList.get(0),getString(R.string.ResourceWordTran),"这是“世界”经历大战后，古人遗留下来的魔力凝结的物质。是极佳的素材。被各国的商人们高价收购。据说法术学院当中的那些“帽子”们，在它身上发现了重大的秘密。");
            NewItemColumn(RareItemList.get(0),getString(R.string.ResourceWordTran),"自精灵一族居住的森林当中获得的树木。就产量而言，这一材料并不稀有。真正让它具备价值的原因，在于精灵族对森林的守护。能从精灵森林的重重陷阱当中逃脱的伐木工可为数不多。");
            NewItemColumn(RareItemList.get(1),getString(R.string.ResourceWordTran),"装有流动元素的玻璃瓶，是缺乏魔力者和初学者使用魔法的重要道具。只有熟练的炼金术师和法师们联合才能制作。且其市场流通被严格监管，供不应求。");
            NewItemColumn(DecentItemList.get(0),getString(R.string.ResourceWordTran),"蕴含灵气的矿石，可以从中提取出编制魔法术式的能量，或是提炼过后，作为机械的动力源。是一种广泛使用的资源。不过，矿石本身其实价值有限，其珍贵实际上体现在被提炼的纯度上。");
            NewItemColumn(DecentItemList.get(1),getString(R.string.ResourceWordTran),"通过精心清洁和祝福的绳子，相较于它“弱不禁风”的外观，它的束缚力其实很大。一些贵族也因为其纯白无暇的外观而将这种绳子用于装饰。是一种兼具实用和装饰性的资源。");
            NewItemColumn(NormalItemList.get(0),getString(R.string.ResourceWordTran),"经过铁匠锻造，塑性的优质金属。是被用于制作武器，防具等用途的素材。注意，虽然名字叫“韧铁”，但不一定是金属铁，也可能是其他物质的混合体。打造这些金属的细节，是各个地区的铁匠们不约而同要保守的秘密。");
            NewItemColumn(NormalItemList.get(1),getString(R.string.ResourceWordTran),"金银等贵金属具备良好的魔法适应性，是用于制作法器，魔法制品的好素材。“蚀金”就是通过炼金术师之手刻蚀的黄金。其上的刻痕可以是魔力的流动更为畅通，但有市井传言道：真正优良的“蚀金”是另有制作方法的。这可能就是普通的“蚀金”明明含有不少黄金，价值却平平无奇的原因吧。");
        }
    }
    ...
    public void OpenMarketItem(View view){
        ...
        NumberView.setHint("Input number you want to Open.");
        //2.show dialog to user.
        AlertDialog.Builder dialog = new AlertDialog.Builder(this);
        dialog.setTitle("You are Opening Box now.\n");
        ...
        dialog.setPositiveButton(
                ...
                (dialog1, id) -> {
                    ...
                    try {
                        OpenNumber = Integer.parseInt(NumberView.getText().toString());
                    ...
                    while (OpenTime > 0){
                        ...
                        //4.3 change the number and save changes.
                    ...
                    //5.finish all work.
                    dialog1.cancel();
                    SupportClass.CreateNoticeDialog(this,"The item you got:",GotItemList.toString(),getString(R.string.ConfirmWordTran));
                });
    ...
    public void BuyMarketItem(View view){
        ...
        final EditText NumberView = new EditText(this);
        NumberView.setInputType(InputType.TYPE_CLASS_NUMBER);
        NumberView.setHint("Input number you want to Buy.");
        //2.show dialog to user.
        AlertDialog.Builder dialog = new AlertDialog.Builder(this);
        dialog.setTitle("You are buying [" + ItemType + "] now.\n");
        dialog.setCancelable(true);
        dialog.setView(NumberView);
        dialog.setPositiveButton(
                getString(R.string.ConfirmWordTran),
                (dialog1, id) -> {//3.do buying work.
                    int BuyNumber = getInputNumber(NumberView);
                    if(ItemType.equals("Box") && UserPoint >= BoxMarketPrice * BuyNumber){
                        CostPoint(BoxMarketPrice * BuyNumber);
                        GetBox(BuyNumber);
                    }else if(ItemType.equals("Key") && UserPoint >= KeyMarketPrice * BuyNumber){
                        CostPoint(KeyMarketPrice * BuyNumber);
                        GetKey(BuyNumber);
                    }else{
                        Toast.makeText(getApplicationContext(), "Not enough point to buy.", Toast.LENGTH_SHORT).show();
                    }
                    dialog1.cancel();
                });
        dialog.setNegativeButton(getString(R.string.CancelWordTran),
    ...
    public void SellMarketItem(View view){
        ...
        final EditText NumberView = new EditText(this);
        NumberView.setInputType(InputType.TYPE_CLASS_NUMBER);
        NumberView.setHint("Input number you want to Sell.");
        //2.show dialog to user.
        AlertDialog.Builder dialog = new AlertDialog.Builder(this);
        dialog.setTitle("You are selling [" + ItemType + "] now.\n");
        dialog.setCancelable(true);
        dialog.setView(NumberView);
        dialog.setPositiveButton(
                getString(R.string.ConfirmWordTran),
                (dialog1, id) -> {//3.do selling work.
                    int CostNumber = getInputNumber(NumberView);
                    if(ItemType.equals("Box") && UserBoxNumber >= CostNumber){
                        CostBox(CostNumber);
                        GetPoint(BoxMarketPrice * CostNumber);
                    }else if(ItemType.equals("Key") && UserKeyNumber >= CostNumber){
                        CostKey(CostNumber);
                        GetPoint(KeyMarketPrice * CostNumber);
                    }else{
                        Toast.makeText(getApplicationContext(), "not enough item to sell.", Toast.LENGTH_SHORT).show();
                    }
                    dialog1.cancel();
                });
        dialog.setNegativeButton(getString(R.string.CancelWordTran),
    ...
    private int getInputNumber(EditText NeedToGet){
