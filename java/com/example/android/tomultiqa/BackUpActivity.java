...
public class BackUpActivity extends AppCompatActivity {
    ...
    //time class, thanks to: https://www.cnblogs.com/travellife/p/3945240.html !
    public void StartExportData(View view) throws IOException {
        ...
        //2.3 write actual content to file.(line4 and above)
        ...
        //3.Content import finished, start writing .txt file.
        String strFilePath = BackUpFilePath + BackUpFileName;
        File file = new File(strFilePath);
        if (!file.exists()) {//3.1 if file doesn't exist, will create a new file.
            file.getParentFile().mkdirs();
            file.createNewFile();
        }
        //3.2 transform String to bytes and put in file.
        RandomAccessFile raf = new RandomAccessFile(file, "rwd");
        raf.seek(0);//write texts from starting position, it will wipe existing texts where the newer text should be there.
        //raf.seek(file.length());//continue existing texts to write, but we don't need it now, so I leave it here, don't mind it.
        raf.write(ContentNeedToAdd.getBytes());
        raf.close();
        //4.finished, after Export operation.
    ...
    public void StartImportData(View view){
        ...
        if(IsFileExist){
            ...
            //compatible with Old version files (before 0.31.0), to prevent NPE.
            ...
            SupportClass.saveIntData(this,"TourneyDataFile","BossLevel",getPureInt(TextElement.get(31)));
            SupportClass.saveIntData(this,"TourneyDataFile","BossHP",getPureInt(TextElement.get(32)));
            SupportClass.saveIntData(this,"TourneyDataFile","BossTurn",getPureInt(TextElement.get(33)));
            ...
            SupportClass.CreateNoticeDialog(this,
                    ...
                    getString(R.string.FileNotExistTran),
                    getString(R.string.ConfirmWordTran));
        }
        StopButtonLock();
    }
    ...
    //lv.1 method, sub method of StartImportData() method. Get all text in .txt file and return them.
    ...
    //lv.1 method, read each line's text in .txt file,and add each line's text to ArrayList to write in App.
